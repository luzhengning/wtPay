...
{
    public class AnalysisBaseWT : AnalysisInlet
    {
        ...
        /// </summary>
        /// <param name="rd"></param>
        private void varRightBcd(byte[] dataStr, ResultData rd, int len)
        {
        ...
        /// <param name="rd"></param>
        /// <param name="len"></param>
        private void varAsciiStr(byte[] dataStr, ResultData rd, int len)
        {
            int datalength = Convert.ToInt32(PosProtocol.bcd2Str(PosProtocol.subbyte(dataStr, rd.pos, len)));
            rd.pos += len;
            //如果长度大于 已知位数，则截取掉最后多余的位数
